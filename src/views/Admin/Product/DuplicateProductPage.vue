...
const currentPageTitle = ref('Dupliquer un produit')
...
const dimensionsInput = ref({
  width: product.value.dimensions.width || null,
  height: product.value.dimensions.height || null,
  depth: product.value.dimensions.depth || null
})
...
const loadProductToDuplicate = async (productId) => {
  try {
    ...
    const formatDate = (dateString) => {
      ...
      return date.toISOString().split('T')[0] // Format YYYY-MM-DD pour les inputs date
    }

    // Fonction utilitaire pour parser les dimensions
    const parseDimensions = (dimensions) => {
      if (typeof dimensions === 'string') {
        try {
          return JSON.parse(dimensions)
        } catch (e) {
          console.warn('Erreur parsing dimensions:', e)
          return {}
        }
      }
      return dimensions || {}
    ...
    // Mise à jour du produit avec gestion correcte des types
    product.value = {
      ...
      brand_id: productData.brand_id ? parseInt(productData.brand_id) : null,
      gender: productData.gender || 'unisex',

      // Stock & Précommande avec conversion booléenne
      is_preorder: Boolean(productData.is_preorder),
      preorder_deadline: formatDate(productData.preorder_deadline),
      expected_ship_date: formatDate(productData.expected_ship_date),

      // Prix avec conversion numérique
      ...
      is_flash_sale: Boolean(productData.is_flash_sale),
      ...
      weight_grams: parseInt(productData.weight_grams) || 0,
      dimensions: parseDimensions(productData.dimensions),
      country_of_origin: productData.country_of_origin || '',

      // Visibilité avec conversion booléenne
      is_featured: Boolean(productData.is_featured),
      is_trending: Boolean(productData.is_trending),

      // Dates
      release_date: formatDate(productData.release_date),
      vintage_year: productData.vintage_year ? parseInt(productData.vintage_year) : null,

      // Images et variantes (remise à zéro pour duplication)
      images: [], // Nouvelles images à uploader
      variants: productData.variants ? [...productData.variants] : []
    }

    // Mise à jour des dimensions dans l'interface
    const dims = parseDimensions(productData.dimensions)
    ...
    console.log('Produit à dupliquer chargé:', product.value)
    console.log('Dimensions parsées:', dimensionsInput.value)
    ...
    validationErrors.value = ['Erreur lors du chargement du produit à dupliquer']
  }
}

// 2. Correction du watcher pour les dimensions avec debounce
import { debounce } from 'lodash-es' // ou utiliser votre propre implémentation

const updateDimensions = debounce((newDimensions) => {
  product.value.dimensions = { ...dimensionsJsonPreview.value }
}, 300)

// Remplacer le watch existant par:
watch(dimensionsInput, updateDimensions, { deep: true })

// 3. Amélioration du computed pour les dimensions
const dimensionsJsonPreview = computed(() => {
  const dims = {}
  if (dimensionsInput.value.width && dimensionsInput.value.width > 0) {
    dims.width = parseFloat(dimensionsInput.value.width)
  }
  if (dimensionsInput.value.height && dimensionsInput.value.height > 0) {
    dims.height = parseFloat(dimensionsInput.value.height)
  }
  if (dimensionsInput.value.depth && dimensionsInput.value.depth > 0) {
    dims.depth = parseFloat(dimensionsInput.value.depth)
  }
  return dims
})
...
const submitForm = async () => {
  ...
  try {
    ...
    const dimensions = dimensionsJsonPreview.value
    ...
    Object.keys(productData).forEach(key => {
      ...
      if (value !== undefined && value !== null && value !== '') {
        if (key === 'dimensions' && typeof value === 'object') {
          // S'assurer que les dimensions sont valides avant envoi
    ...
    product.value.variants.forEach((variant, index) => {
      ...
      if (variant.barcode) {
      formData.append(`variants[${index}][barcode]`, generateBarcode())
      }
...
const generateBarcode = () => {
  return 'BARCODE-' + Math.floor(Math.random() * 1000000000);
}

const debugProductData = (productData) => {
  console.group('🔍 Debug Produit Reçu')
  console.log('Données brutes:', productData)
  console.log('Type des dimensions:', typeof productData.dimensions)
  console.log('Contenu dimensions:', productData.dimensions)
  console.log('Dates brutes:', {
    preorder_deadline: productData.preorder_deadline,
    expected_ship_date: productData.expected_ship_date,
...
const validateDimensions = () => {
  const dims = dimensionsJsonPreview.value
  const hasAllDimensions = dims.width && dims.height && dims.depth
  const allPositive = dims.width > 0 && dims.height > 0 && dims.depth > 0

  if (!hasAllDimensions) {
    return 'Toutes les dimensions sont requises (largeur, hauteur, profondeur)'
  }
  if (!allPositive) {
    return 'Toutes les dimensions doivent être supérieures à 0'
  }
  return null
}
